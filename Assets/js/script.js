//ensure html page is loaded before JavaScript
$(document).ready(function() {
    ...
    var currentHour = moment().hour(); // used to set the color of the scheduled time-blocks
    ...
    // function to retrieve data from local storage and return it, or an empty array
    function getDaysEvents() {
        var daysEvents = localStorage.getItem("scheduledEvents"); // retrieve data from local storage
        var tempSchedEvents = ["","","","","","","","",""];
    ...
    function eventSave(event) {
        ...
        if ($("#textSchedule"+index).val() !== tempSchedEvents[index]) {
            ...
            //delay long enough for the user to see the saved schedule message before deleting it
            setTimeout(() => {
                $("#msgSaved").empty();
            }, 1000);
        }
